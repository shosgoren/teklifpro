...
async function POST(
  ...
  if (
    ...
    throw badRequest('Missing required fields: actionType, entityType, entityId, details');
  }

  // Validate action type
  const validActions = ['CREATE', 'UPDATE', 'DELETE', 'SEND', 'LOGIN'];
  const actionType = body.actionType.toUpperCase();
  if (!validActions.includes(actionType)) {
  ...
  // Sanitize inputs
  const sanitizedDetails = sanitizeInput(body.details);
...
export const DELETE = withErrorHandling(DELETE);
