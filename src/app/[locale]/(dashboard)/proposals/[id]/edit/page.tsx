              ...
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    <Skeleton className="w-12 h-12 rounded-xl mb-1.5" />
                    <Skeleton className="h-3 w-16" />
                  </div>
                  {i < 4 && <Skeleton className="flex-1 h-1 mx-3 rounded-full" />}
                </div>
              ))}
            </div>
          </div>
...
export default function EditProposalPage() {
  ...
  useEffect(() => {
    if (!proposalData?.data || isDataLoaded) return

    const proposal = proposalData.data
    const customer = proposal.customer

    // Calculate validity days from expiresAt
    let validityDays = 30
    ...
    const formItems: ProposalFormData['items'] = (proposal.items || []).map((item: { productId?: string; id: string; name: string; quantity: number; unitPrice: string | number; discountRate?: string | number; vatRate?: string | number }) => ({
      ...
      discountFixed: 0,
      vatPercent: Number(item.vatRate) || 18,
    }))

    // Reset entire form with loaded data
    reset({
      customer: {
        id: customer?.id || '',
        name: customer?.name || '',
      ...
      proposalType: proposal.proposalType || 'OFFICIAL',
      items: formItems,
      ...
      voiceNoteDuration: proposal.voiceNoteDuration || null,
    })

    // Preserve the original status so we don't change it on update
    setProposalStatus(proposal.status || 'DRAFT')

    if (customer) {
      setSelectedContact({ id: customer.id, name: customer.name })
    }

    setIsDataLoaded(true)
  }, [proposalData, isDataLoaded, reset])

  useEffect(() => {
    if (isDirty && isDataLoaded) markDirty()
  ...
  // Update proposal via PUT
  const updateProposal = async (data: ProposalFormData): Promise<string> => {
  ...
  const sendProposal = async (id: string, method: 'whatsapp' | 'email') => {
    const res = await fetch(`/api/v1/proposals/${id}/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveAndSend = async (method: 'draft' | 'whatsapp' | 'email') => {
    ...
    try {
      const updatedId = await updateProposal(formData)

      if (method === 'whatsapp' || method === 'email') {
        const sendResult = await sendProposal(updatedId, method)
        const channel = method === 'whatsapp' ? 'WhatsApp' : 'E-posta'
        toast.success(`${t('proposals.updated')} — ${channel}: ${sendResult.sentTo}`)
      } else {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center space-y-4">
            <div className="w-16 h-16 rounded-2xl bg-red-100 dark:bg-red-950 flex items-center justify-center mx-auto">
              <FileText className="h-8 w-8 text-red-500" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-red-600">{t('proposals.error')}</h2>
              <p className="text-sm text-muted-foreground mt-1">
                {fetchError.message || 'Teklif yuklenemedi.'}
              </p>
            </div>
            <Button
              variant="outline"
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-1">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push(`/${locale}/proposals/${proposalId}`)}
